...
def Ok():
	...
	if results:
		...
		call(["python", "example/vehicle.py"])
...
rot_i = Image.open("image/Frame1.png")
rotunda = ImageTk.PhotoImage(rot_i)
label2 = tk.Label(image=rotunda,bg='#384e5d')
label2.image = rotunda
label2.place(x=430,y=255)

#img = ImageTk.PhotoImage(Image.open("image/Frame1.png"))

frame_login=Frame(root, bg='#95baa8', highlightbackground="black", highlightthickness=2).place(height=40,width=1400,x=0,y=0)
root.attributes('-fullscreen',True)
#------old color 95baa8
#frame_login=Frame(root, bg='#384e5d', highlightbackground="black", highlightthickness=2).place(height=220,width=530,x=430,y=255)

#------------------------
#title=Label(frame_login ,text="login here",font=("Impact",35,"bold"),fg='#d77337',bg='#384e5d').place(x=600,y=260)
...
login_button = Button(root,  text="Login", command= lambda:[Ok()], relief="ridge",width = 9,height =1,font=("times new roman",12,"bold")).place(x=600, y=400)
# changeOnHover(login_button, "red", "yellow")
#-------------------Label Time----------------
def my_time():
