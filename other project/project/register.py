...
mycursor.execute('''if  exists (Select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'record') 
                begin 
                CREATE TABLE record (
                    name nvarchar(100), 
                    email nvarchar(100), 
...
def insert_record():
    ...
    if check_counter == 8:        
        try:
            con = pyodbc.connect('test.db')
            mycursor = con.cursor()
            mycursor.execute("insert into record (name, email, contact, gender, country, password) values ('{}', '{}', '{}','{}', '{}', '{}')".
                       format(register_name, register_email, register_mobile,var,variable,register_pwd))
            # mycursor.execute("INSERT INTO record VALUES (name, email, contact, gender, country, password)", {
...
register_btn = Button(right_frame, width=15, text='Register', font=f, relief=SUNKEN,cursor='hand2',command=insert_record)
